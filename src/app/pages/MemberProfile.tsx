	...
	return (
		<div className="min-h-screen max-w-5xl mx-auto">
			<div className="header">
				...
				<div className="py-2">
					<h1 className="py-2 font-semibold text-2xl uppercase">
						{member.name}
					</h1>
					<p className="text-slate-600 py-2">
						{member.dept + " (" + member.level + ")"}
					</p>
			...
			<header className="w-full py-4 my-2 border-b text-slate-600">
				<div className="flex flex-row flex-wrap gap-8 [&>*]:p-1">
					<NavLink
						...
						className={({ isActive }) =>
							isActive
								? " border-secondary-900 border-b-4 hover:bg-slate-100 rounded-t-md"
								: "border-b-transparent border-b-4 hover:bg-slate-100 rounded-md"
						}
						end
					>
						Education
					...
					<NavLink
						to={"/members/" + memberId + "/contributions"}
						className={({ isActive }) =>
							isActive
								? "border-b-4 pb-1 border-secondary-900 hover:bg-slate-100 rounded-t-md"
...
export function Education() {
	...
	const formatedQualifications = eduction.map((q, k) => (
		<Certificate cert={q} key={k} />
	));
	return (
		<div className="my-8 divide-y [&>*]:py-4">{formatedQualifications}</div>
	);
}
...
export function Contributions() {
	...
	const member = useAppSelector(
		(state) => state.data.members.filter((member) => member.id === id)[0]
	);
			...
			<tr key={key} className="relative [&>*]:py-4 hover:bg-slate-100">
				<td className="hidden sm:block">
					<img
						...
						className="h-8 w-12 rounded-md border object-center object-cover"
						alt=""
					/>
				</td>
				<td>{contribution.projectTitle}</td>
				<td className="hidden sm:flex flex-row">
					{contribution.contributors.map((member, key) => (
						<img
							key={key}
							className="rounded-full h-8 border hover:scale-105"
							src={member.profileImage}
							alt={member.name}
						/>
					))}
				...
				<td>
					<span className="px-4 py-1 rounded-full text-sm bg-secondary-50 text-slate-600 capitalize">
	...
	return (
		<div className="my-5">
			<table className="w-full [&>*]:divide-y text-s">
				...
				<tbody className="[&>tr>*]:px-4">{formated}</tbody>
			</table>
		</div>
	);
}
...
function Certificate(props: { cert: Qualification }) {
	return (
		<div>
			<h1 className="my-1 text-lg text-primary-900 font-semibold">
				{props.cert.degree}
			</h1>
...
export function ContactInfo(): JSX.Element {
	...
	const member = useAppSelector((state) =>
		state.data.members.find((m) => m.id === memberId)
	);

	const contacts = member?.contacts;
	const emails = contacts?.emails.map((email, key) => (
		<li key={key}>{email}</li>
	));
	const phones = contacts?.phones.map((phone, key) => (
		<li key={key}>{phone}</li>
	));
	const addresses = contacts?.addresses.map((address, key) => (
			...
			<div className="rounded-md w-full shadow-inner border p-4 my-8">
				...
				<div className="flex flex-col gap-4 sm:flex-row">
					...
					<div className="form flex-1">
						...
						<button className="py-2 px-4 my-2 text-sm bg-primary-900 hover:bg-primary-800 hover:scale-[1.01] text-slate-50 rounded-md focus:ring-2 ring-primary-900 ring-offset-1s">
							Send message
						</button>
					</div>
				</div>
...
function Social() {
	const memberId = Number(useParams().memberId);
	const socials = useAppSelector((state) =>
		state.data.members.find((m) => m.id === memberId)
	)?.mediaLinks;
	return (

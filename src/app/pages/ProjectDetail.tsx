...
function PojectDetail() {
	...
	const project = useAppSelector((state) => state.data.projects).filter(
		...
		<div className="py-8 min-h-screen flex flex-col md:flex-row gap-x-10">
			...
			<div className="flex-1">
				<div>
					...
					<span className="text-sm ml-2 px-4 py-1 bg-secondary-50 rounded-full capitalize">
						{project.status}
					</span>
					...
					<div className="my-2">
						...
						<div className="flex flex-row my-2">{contributors}</div>
					</div>
				</div>
			</div>
		</div>

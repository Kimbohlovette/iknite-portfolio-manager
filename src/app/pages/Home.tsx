...
function Home() {
	const shortProjects = useAppSelector((state) => state.data)
		.projects.filter((project) => project.contributors.length > 2)
		.map((project) => {
			return {
	...
	const leadEngineers = useAppSelector((state) => state.data)
		.members.filter((member) => member.level.toLocaleLowerCase() === "lead")
		.map((member, key) => {
	...
	return (
		<div className="min-h-screen">
				...
				<div className="grid grid-cols-1 sm:grid-cols-2 py-4">
					<div className="text flex justify-start items-center min-h-[45vh]">
						<div>
							...
							<span className="inline-block my-4 w-3/4 font-bold text-slate-700">
								Get to know more about Iknite Space talents and
								their projects here.
							</span>
							<Link to="/members">
								<button className="block text-sm py-2 px-4 my-4 bg-secondary-900/80 backdrop-blur-sm text-primary-800 rounded-md shadow-sm hover:bg-secondary-800">
									<Icon
					...
					<div className="hidden sm:flex caption justify-center items-center">
						<img
							src={studentImage}
							className="object-cover object-center aspect-square w-3/4"
							alt=""
						/>
					</div>
				</div>
			...
			<section className="py-5">
				<h1 className="pb-5 px-2 max-w-xs text-slate-800 text-2xl sm:text-3xl lg:text-4xl font-semibold">
					Popular Projects
				</h1>
			...
			<section className="py-5">
				<h1 className="rounded-md py-4 px-2 my-8 max-w-xs  text-2xl sm:text-3xl lg:text-4xl font-semibold">
					Senior Engineers
				</h1>
				<div className="cards flex flex-wrap gap-4 [&>*]:max-w-xs justify-center sm:justify-start">
					{leadEngineers}
			...
			<section className="py-5">
				<h1 className="py-4 px-2 my-5 max-w-xs text-slate-800 text-2xl sm:text-3xl lg:text-4xl font-semibold">
					Top News & Events
				</h1>
	...
	return (
		<div className="flex flex-col [@media(min-width:520px)]:flex-row gap-2 p-4 rounded-md max-w-3xl hover:bg-slate-100 cursor-pointer">
			<div className="caption w-full max-w-xs">
				<img
					src="https://i0.wp.com/www.afrohustler.com/wp-content/uploads/2017/11/feature.jpg?fit=770%2C513&ssl=1"
					className="max-h-[12rem] aspect-video object-center object-cover overflow-hidden rounded-md"
					alt=""
				/>
			</div>
			<div className="text p-2 rounded-md">
				<div>
					<h1 className="text-lg font-semibold text-slate-800">
						SMCON23 - Silicon Mount Conference 2023
					</h1>
					<p className="text-slate-600 text-sm p-2 rounded-md">
						Lorem ipsum dolor sit amet, consectetur adipisicing
						elit. Laboriosam neque harum adipisci atque odio...
					</p>
				</div>
				<div className="date italic">November 11, 2023</div>
			</div>
		</div>
	);
}
	...
	return (
		<div className="card w-full sm:max-w-xs rounded-md p-4 shadow-lg border cursor-pointer">
			...
			<Link to={"/projects/" + props.project.projectId}>
				<button className="text-sm bg-primary-800 hover:bg-primary-300 hover:text-slate-800 py-2 my-2 w-1/2 text-slate-100 rounded-md font-medium">
					View
				</button>
			</Link>
		</div>
